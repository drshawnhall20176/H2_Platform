...
# lineup slot to).
BATTER_MARKETS = ["Batter HR", "Batter Total Hits", "Batter Total Bases", "H+R+RBI",
...
def run_analysis(sport_key: str = "MLB", markets: Optional[List[str]] = None,
                 big_game_hits: int = 2, min_sample: int = 20) -> Dict:
    """The real, end-to-end analysis. Returns {"hot_neighbor": {"n", "hit_rate"},
    "cold_neighbor": {"n", "hit_rate"}, "no_neighbor_data": int, "skipped_no_game": int} --
    every real count reported honestly, including the real plays this couldn't check at all,
...
def main() -> int:
    ...
    result = run_analysis(args.sport, markets, args.big_game_hits, args.min_sample)
