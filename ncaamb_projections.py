...
import odds_api as O   # for real_entry_price/real_market_prob -- real price/reference wiring,
                       # same functions MLB/WNBA/NBA's own build_best_bets already use
...
def build_best_bets(rows: List[Dict], sims: int = DEFAULT_SIMS,
    ...
    REAL DATA, WIRED IN DIRECTLY, MATCHING WNBA/NBA'S OWN PATTERN -- real_lines/offers/
    preferred_book used to be accepted "for signature parity" only and never actually used,
    meaning every NCAAMB play always priced off the _MARKET_SPEC placeholder regardless of what
    load_generic_best_bets_board had already fetched. All three now wired in, same as MLB/WNBA/NBA:
      - Line: the real book line when real_lines supplies one for this player/market (NCAAMB's
    ...
    for r in rows:
        ...
        for mkey, (col, disp, default_line) in _MARKET_SPEC.items():
            ...
            # otherwise -- same real distinction MLB/WNBA/NBA's own Conviction fix already
            # established.
