...
def _clip_prob(p: float) -> float:
    ...
    certainty. Two reasons this matters: (1) claiming 100%/0% is overconfident for a v1
    recent-form-only model, (2) `prob_to_american` returns None at the exact boundary, which
    breaks a strict `{:+d}` format string wherever a Fair price gets displayed."""
...
def default_board_from_index(index: Dict) -> List[Dict]:
    ...
    Over/Under, no MLB-style Yes/No special case needed here)."""
    out: List[Dict] = []
...
def build_best_bets(rows: List[Dict], sims: int = DEFAULT_SIMS,
    ...
    render any sport's plays through the same code."""
    rng = np.random.default_rng(seed)
    plays: List[Dict] = []
