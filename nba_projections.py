"""
nba_projections.py — turns nba_engine's slate rows into priced probabilities.

Matches projections.py's OUTPUT CONTRACT exactly (build_projection_index / default_board_from_index
/ DEFAULT_SIMS), which is what lets Edge Board consume MLB, WNBA, and NBA through the same code
path via sports.active().projections — see sports.py and views/3_..._Edge_Board.py. The genuinely
...
baseball-specific) are imported straight from projections.py rather than re-implemented here — the
same convention wnba_projections.py already established.

BUILT AS A COPY-ADAPT OF wnba_projections.py — see basketball_engine.py's module docstring for the
extraction plan this follows. The genuinely NBA-specific difference from wnba_projections.py is
_MARKET_SPEC's default lines: NBA games run 48 minutes at a faster pace than WNBA's 40, so counting
stats run meaningfully higher across the board. These are model-only-board fallback defaults (used
only when no live odds have been fetched), not calibrated book numbers, same caveat WNBA's own
defaults carry.

METHOD (deliberately simple — a v1, documented as such): each of a player's last N games
(config_nba.RECENT_GAMES_N) for Points/Rebounds/Assists/Threes Made is treated as one draw from
their true talent distribution. The projection is an empirical bootstrap: resample those games
with replacement `sims` times and use the resulting distribution directly. Known limitation: with
a short game log (early season, new team), the bootstrap can't see tail outcomes the player hasn't
produced yet in that sample. Opponent defensive strength and pace are NOT incorporated into the
...
import basketball_projections as BB_P  # league-agnostic basketball logic, shared with wnba_projections.py
import odds_api as O   # for real_entry_price/real_market_prob -- real price/reference wiring,
                       # same functions MLB and WNBA's own build_best_bets already use

DEFAULT_SIMS = 10000

# Odds-API-market-key -> (row column, display name, default line for the model-only board).
# NBA-SCALE, not WNBA-scale: 48-minute games at a faster pace push counting stats meaningfully
# higher than WNBA's 40-minute games. These are round-number model-only-board fallbacks (used
# only before a live line is fetched), not calibrated book numbers — worth tuning empirically
# once real NBA slate data is available, same honest caveat WNBA's own defaults carry.
_MARKET_SPEC = {
    "player_points":   ("PTS",  "Points",      22.5),
    "player_rebounds": ("REB",  "Rebounds",    7.5),
    "player_assists":  ("AST",  "Assists",     5.5),
    "player_threes":   ("FG3M", "Threes Made", 2.5),
}
...
def default_board_from_index(index: Dict) -> List[Dict]:
    ...
    to projections.default_board_from_index (every NBA market in _MARKET_SPEC is a plain
    ...
    clipped — a 4-game "perfect" streak and a 40-game one no longer produce the identical number."""
    out: List[Dict] = []
...
# --------------------------------------------------------------------------- Best Bets
# Reference (typical/coin-flip) hit-rate per market. All four NBA markets use 0.5 rather than a
# calibrated figure — the default lines themselves (_MARKET_SPEC above) are round-number
...
def _player_reasons(values: List[float], line: float, side: str) -> str:
    ...
    avg = sum(values) / n
    recent = values[:3]                       # most recent first (see nba_engine.get_player_recent_games)
...
def explain_miss(row: Optional[Dict], market: str = "Points") -> str:
    """NBA equivalent of retro.explain_miss's role: explain a result the model ranked LOW. `row`
    is a build_slate row looked up by player id; None means the player wasn't on the projected
    slate at all (below the rotation-minutes bar, or a late addition the model never saw)."""
    if not row:
...
def build_best_bets(rows: List[Dict], sims: int = DEFAULT_SIMS,
    ...
    REAL DATA, WIRED IN DIRECTLY, MATCHING WNBA'S OWN PATTERN -- real_lines/offers/preferred_book
    used to be accepted "for signature parity" only and never actually used, meaning every NBA
    play always priced off the _MARKET_SPEC placeholder regardless of what
    load_generic_best_bets_board had already fetched. All three now wired in, same as MLB/WNBA:
      - Line: the real book line when real_lines supplies one for this player/market (NBA's
        _MARKET_SPEC keys are already the real Odds API market keys -- "player_points" etc --
        so no separate market-name mapping is needed), the _MARKET_SPEC placeholder otherwise
        (LineSource records which).
    ...
    for r in rows:
        ...
        for mkey, (col, disp, default_line) in _MARKET_SPEC.items():
            ...
            # otherwise -- same real distinction MLB/WNBA's own Conviction fix already established.
...
# NBA-scale blowout threshold: higher-scoring, faster-paced games than WNBA's mean a "big
# mismatch" spread runs larger in absolute points. 12.0 vs WNBA's 10.0 is a starting adjustment,
# not a backtested cutoff — worth tuning empirically once real NBA slate data is available, same
# honest caveat every other tuning constant in this build carries.
...
def build_hot_hand_board(rows: List[Dict], opp_allowed: Dict[int, Dict[str, float]],
    ...
    {team_id: {pts,reb,ast,fg3m,poss}} from nba_engine.get_team_recent_allowed_stats, one call
    ...
    (identical here; this is a copy-adapt, not a different design).

    `team_rest` (optional) is {team_id: {rest_days, is_back_to_back, ...}} from nba_engine.
    ...
    deliberately NOT folded into Matchup Factor/Score — same philosophy as WNBA's board.
...
def build_matchup_profile(row: Dict, h2h_log: List[Dict], opp_allowed: Dict[str, float],
    ...
    (identical here; copy-adapt, not a different design). One NBA-specific note: h2h_log can
    legitimately be empty or small even mid-season — NBA teams play each other 2, 3, or 4 times a
    season depending on conference/division, not the uniform "meet 2-4 times" WNBA's balanced
    schedule produces, so a genuinely small H2H sample is common and expected here too, reported
