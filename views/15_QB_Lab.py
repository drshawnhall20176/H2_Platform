...
st.caption("Both Proj columns already account for how much yardage that specific opponent's "
          ...
          "Rush Yards is shown here even though QBs don't get a shared Rush Yards market on Edge "
          "Board/Best Bets — that exclusion was about not mixing a scrambling QB's carries with "
          "a workhorse RB's volume under one shared betting line; there's no such conflict here.")
if matchup_proj:
    pdf = pd.DataFrame(matchup_proj)
    sort_mode = st.radio("Sort", ["Projected Pass Yds", "Projected Rush Yds", "Matchup Factor"],
                         horizontal=True, key="qb_proj_sort")
...
if efficiency:
    ...
    for _, r in movers.head(5).iterrows():
        ...
        st.code(
            ...
            f"worth watching whether that holds up or reverts. #NFL",
            language=None,
        )
else:
