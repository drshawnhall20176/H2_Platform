...
def _board_mlb(date_str):
    ...
    # Statcast loaded separately here specifically for the "ev_mode" feature's own
    # build_projection_index call below, which build_mlb_board doesn't expose internally. Not a
    # new parallel pipeline -- rows/meta/plays all still come from the one real, shared source;
    # this is the one piece build_mlb_board's own return doesn't carry. SC.load_cached() (not a
    # local wrapper redefined here) so this shares ONE real cache entry platform-wide -- see that
    # function's own docstring for the real, confirmed finding this fixes (this exact page was
    # one of 6 places independently re-parsing the same file from disk, on its own separate hour-
    # long clock).
    sc, k = SC.load_cached()
